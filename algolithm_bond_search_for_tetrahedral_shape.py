...
def filter_2(df_nnlist, central_atom_symbol, neighboring_atom_symbol, bond_length_lower_end, bond_length_upper_end):
    """
    2．POSCAR.nnlistにおいて，原子'N'から0-2．のNH結合距離以内に，原子'H'を4つ含む，中心原子'N'が存在するかどうか判定．
    → 存在する場合、True値，{中心原子'N'の'central_atom_id': そのneighborsの'central_atom_id'}の辞書の2つを返す．
    ...
    for key in df_nnlist_central_atom_ids:
        ...
        bool_list.append(df_nnlist.iloc[df_nnlist_group_dict[key]].sort_values('rel_distance')[bool_sort_dist_list]['neighboring_atom_symbol'].tolist().count(neighboring_atom_symbol) == 4)
...
def filter_3(df_nnlist, dict_2, neighboring_atom_symbol):
    """
    3．2．の中心原子'N'に対して，1番近い原子が'H'であり，かつ2番目・3番目・4番目に近い原子も'H'である，中心原子'N'が存在するかどうか判定．
    ...
    for k, v in dict_2.items():
        ...
        bool_list_3.append(set(df_nnlist.iloc[dict_2[k]].sort_values(by='rel_distance')['neighboring_atom_symbol'].tolist()[1:5]) == {neighboring_atom_symbol})
...
def filter_4(df_nnlist, dict_3):
    """
    4．2．の中心原子'N'に対して5番目に近い原子が存在しないかを判定．
    → 存在しない場合，True値，{中心原子'N'の'central_atom_id': そのneighborsの'central_atom_id'}の辞書の2つを返す．
...
def filter_5(df_nnlist, dict_3):
    """
    5．2．の中心原子'N'に対して5番目に近い原子が，'N'に4番目に近い原子'H'と'N'のNH距離より大きい'N'が存在するどうかを判定．
    → 存在する場合，True値，{中心原子'N'の'central_atom_id': そのneighborsの'central_atom_id'}の辞書の2つを返す．
    → 存在しない場合，False値，空の辞書を返す．
...
def filter_6(df_nnlist, dict_3, central_atom_symbol):
    """
    6．3．の4つの原子'H'全てに対して，3．の中心の原子'N'との距離以内に，中心原子'N'以外の別の原子が存在しないかどうかを判定．
    → 存在しない場合，True値，中心原子'N'の'central_atom_id'のndarrayの2つを返す．
    ...
    for k, v in dict_3.items():
        ...
        bool_list_temp = []
        for H_id in H_ids:
            bool_temp = df_nnlist[df_nnlist['central_atom_id'] == H_id].sort_values('rel_distance')['neighboring_atom_symbol'].tolist()[1] == central_atom_symbol
            bool_list_temp.append(bool_temp)
        if set(bool_list_temp) == {True}:
            bool_list_6.append(True)
        else:
            bool_list_6.append(False)
    N_ids = np.array(list(dict_3.keys()))[bool_list_6]
    bool_filter_6 = bool_list_6.count(True) >= 1

    return bool_filter_6, N_ids
...
if __name__ == '__main__':
    ...
    print(concat_filter(df_nnlist, central_atom_symbol='N', neighboring_atom_symbol='H', bond_length_lower_end=0.82, bond_length_upper_end=1.24))
